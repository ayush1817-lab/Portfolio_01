...
export function ProjectsShelf({ id, index, title, caption, items, badge }: Props) {
  ...
  return (
    <section
      id={id}
      aria-label={title}
      className="snap-start border-b border-[color:var(--color-ink)]/10 bg-[color:var(--color-paper)] py-6 md:py-10 lg:py-24"
    >
              ...
              {items.map((p, i) => {
                ...
                return (
                  <Card
                    ...
                    draggable={false}
                    className={cn(
                      "group relative flex h-[240px] w-[62vw] max-w-[220px] shrink-0 flex-col border border-[color:var(--color-ink)] bg-[color:var(--color-paper)] transition-transform duration-300 lg:h-[420px] lg:w-[320px] lg:max-w-[340px]",
                      p.link && "hover:-translate-y-1",
                    )}
                  >
                    ...
                    <footer className="flex items-center justify-between border-t border-[color:var(--color-ink)] bg-[color:var(--color-ink)] px-4 py-2 font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-[color:var(--color-paper)]">
                      <span>{p.link ? "Open dossier" : "Overview only"}</span>
                      {p.link ? (
